...
export function PaymentSettingsSection({
  ...
  // WeChat Native payment notify URLs are generated by the backend at order
  // time as `{callbackBase}/api/wechat/notify` (wallet) and
  // `{callbackBase}/api/subscription/wechat/notify` (subscription), where
  // callbackBase = CustomCallbackAddress if set, otherwise the server address.
  // These are display-only; nothing needs to be configured on the WeChat
  // merchant platform for Native payment notifications.
  const wechatCallbackBase = removeTrailingSlash(
    currentFormValues.CustomCallbackAddress ||
      (typeof window === 'undefined' ? '' : window.location.origin)
  )
  const wechatTopupNotifyURL = `${wechatCallbackBase}/api/wechat/notify`
  const wechatSubscriptionNotifyURL = `${wechatCallbackBase}/api/subscription/wechat/notify`
